...
-(void)onInAppMessageReceived:(id)inAppData withType:(App42IAMDialogueType)type
{
    ...
    App42Action *action = [inAppData app42Action];
    
    NSLog(@"name: %@ action type: %@ action prop: %@", action.actionName, action.actionType, action.actionProperties);
    
    App42Action *cancelaction = [inAppData app42CancelAction];
    
    NSLog(@"name: %@ action type: %@ action prop: %@", cancelaction.actionName, cancelaction.actionType, cancelaction.actionProperties);
